...
//          client.println("Refresh: 5");  // refresh the page automatically every 5 sec
    ...
    client.print("uri: ");
    client.println(uri);
    client.print("<br>reqPage: ");
    client.println(reqPage);
    client.print("<br>ethTxCount: ");
    client.println(ethTxCount);
    client.print("<br>ethRxCount: ");
    client.println(ethRxCount);
    client.print("<br>serialTxCount: ");
    client.println(serialTxCount);
    client.print("<br>serialRxCount: ");
    client.println(serialRxCount);
    client.print("<br>");

    byte countSlaves = 0;
    for (int k = 1; k < maxSlaves; k++) {
      if (getSlaveResponding(k) == true || k == scanCounter) {
        client.print(F("<tr><td><td>0x"));
        if (k < 16) client.print(F("0"));
        client.print(k, HEX);
    ...
    if (countSlaves == 0 && scanCounter == 0) client.print(F("<tr><td><td>none"));
...
//        WiFiClient clientTemp = WiFiClient(n);
...
//        if (action != WEB || (clientTemp.localPort() && clientTemp.localPort() != defaultConfig.webPort && clientTemp.localPort() != defaultConfig.udpPort && clientTemp.localPort() != defaultConfig.tcpPort)) {
//          clientTemp.flush();
//          clientTemp.stop();
//        }
//      }
      switch (action) {
        case WEB:
          webServer = WiFiServer(defaultConfig.webPort);
          break;
        case REBOOT:
          Serial.flush();
          Serial.end();
...
// This function stores POST parameter values in defaultConfig.
...
//        break;
//      case POST_DHCP:
//        if ((byte)paramValueUlong != defaultConfig.enableDhcp) {
//          action = ETH_SOFT;
//          defaultConfig.enableDhcp = (byte)paramValueUlong;
//        }
//        break;
//      case POST_IP ... POST_IP_3:
//        if ((byte)paramValueUlong != defaultConfig.ip[paramKeyByte - POST_IP]) {
//          action = ETH_SOFT;
//          defaultConfig.ip[paramKeyByte - POST_IP] = (byte)paramValueUlong;
//        }
//        break;
//      case POST_SUBNET ... POST_SUBNET_3:
//        if ((byte)paramValueUlong != defaultConfig.subnet[paramKeyByte - POST_SUBNET]) {
//          action = ETH_SOFT;
//          defaultConfig.subnet[paramKeyByte - POST_SUBNET] = (byte)paramValueUlong;
//        }
//        break;
//      case POST_GATEWAY ... POST_GATEWAY_3:
//        if ((byte)paramValueUlong != defaultConfig.gateway[paramKeyByte - POST_GATEWAY]) {
//          action = ETH_SOFT;
//          defaultConfig.gateway[paramKeyByte - POST_GATEWAY] = (byte)paramValueUlong;
//        }
//        break;
//      case POST_DNS ... POST_DNS_3:
//        if ((byte)paramValueUlong != defaultConfig.dns[paramKeyByte - POST_DNS]) {
//          action = ETH_SOFT;
//          defaultConfig.dns[paramKeyByte - POST_DNS] = (byte)paramValueUlong;
//        }
//        break;
//      case POST_TCP:
//        if (defaultConfig.tcpPort != (unsigned int)paramValueUlong) {
//          for (byte i = 0; i < maxSockNum; i++) {
//            EthernetClient clientTemp = EthernetClient(i);
//            if (clientTemp.status() != SnSR::UDP && clientTemp.localPort() == defaultConfig.tcpPort) {
//              clientTemp.flush();
//              clientTemp.stop();
//            }
//          }
//          defaultConfig.tcpPort = (unsigned int)paramValueUlong;
//          modbusServer = EthernetServer(defaultConfig.tcpPort);
//        }
//        break;
//      case POST_UDP:
//        if (defaultConfig.udpPort != (unsigned int)paramValueUlong) {
//          defaultConfig.udpPort = (unsigned int)paramValueUlong;
//          Udp.stop();
//          Udp.begin(defaultConfig.udpPort);
//        }
//        break;
//      case POST_WEB:
//        if (defaultConfig.webPort != (unsigned int)paramValueUlong) {
//          defaultConfig.webPort = (unsigned int)paramValueUlong;
//          action = WEB;
//        }
//        break;
//      case POST_RTU_OVER:
//        defaultConfig.enableRtuOverTcp = (byte)paramValueUlong;
//        break;
//      case POST_BAUD:
//        if (defaultConfig.baud != paramValueUlong) {
//          action = SERIAL_SOFT;
//          defaultConfig.baud = paramValueUlong;
//        }
//        break;
//      case  POST_DATA:
//        if ((((defaultConfig.serialConfig & 0x06) >> 1) + 5) != (byte)paramValueUlong) {
//          action = SERIAL_SOFT;
//          defaultConfig.serialConfig = (defaultConfig.serialConfig & 0xF9) | (((byte)paramValueUlong - 5) << 1);
//        }
//        break;
//      case  POST_PARITY:
//        if (((defaultConfig.serialConfig & 0x30) >> 4) != (byte)paramValueUlong) {
//          action = SERIAL_SOFT;
//          defaultConfig.serialConfig = (defaultConfig.serialConfig & 0xCF) | ((byte)paramValueUlong << 4);
//        }
//        break;
//      case  POST_STOP:
//        if ((((defaultConfig.serialConfig & 0x08) >> 3) + 1) != (byte)paramValueUlong) {
//          action = SERIAL_SOFT;
//          defaultConfig.serialConfig = (defaultConfig.serialConfig & 0xF7) | (((byte)paramValueUlong - 1) << 3);
//        }
//        break;
//      case  POST_TIMEOUT:
//        defaultConfig.serialTimeout = paramValueUlong;
//        break;
//      case  POST_RETRY:
//        defaultConfig.serialRetry = (byte)paramValueUlong;
//        break;
//      case  POST_ACTION:
...
//      {
//        byte tempMac[6];
//        memcpy(tempMac, defaultConfig.mac, 6); // keep current MAC
//        defaultConfig = defaultConfig;
//        memcpy(defaultConfig.mac, tempMac, 6);
//        break;
//      }
//    case MAC:
//      generateMac();
...
////  EEPROM.put(configStart + 1, defaultConfig);    // it is safe to call, only changed values are updated
